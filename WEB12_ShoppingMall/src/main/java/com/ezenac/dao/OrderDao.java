...
public class OrderDao {
	...
	public ArrayList<Integer> selectSeqOrderIng(String id) {
		...
		// order_view 로 부터 주문자의 처리 안된 주문의 주문번호(oseq)를 조회하여 리스트로 리턴합니다
	...
	public ArrayList<Integer> oseqListAll(String id) {
		ArrayList<Integer> list = new ArrayList<Integer>();
		String sql = "select distinct oseq, result from order_view where id=? "
				+ " order by result, oseq desc";  // where 에  result='1' 이 없습니다
